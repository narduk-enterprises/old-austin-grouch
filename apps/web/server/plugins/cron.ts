  ...
  nitroApp.hooks.hook(
    ...
    async (options: unknown) => {
      ...
      try {
        ...
        if (!ai) {
          console.error(
            'Missing AI binding. Make sure it is configured in wrangler.json. Aborting.'
          );
          return;
        }
...
`;
        ...
        let finalContent = markdownContent.replace(
          /date:\s*["']?[^\n"']*["']?/i,
          `date: "${today}"`
        );
        ...
        try {
          ...
          if (imageResponse instanceof ReadableStream) {
            ...
            const bytes =
              imageResponse instanceof ArrayBuffer ? new Uint8Array(imageResponse) : imageResponse;
            let binary = '';
        ...
        const ghResponse = await fetch(postUrl, {
          ...
          headers: ghHeaders,
          body: JSON.stringify({
            message: `feat(content): Automated AI post: ${title}`,
        ...
        console.log(
          `Successfully committed new automated post with hero image! Cron job complete.`
        );
      } catch (error) {
